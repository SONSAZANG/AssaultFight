...
public class Register : MonoBehaviour {
    ...
    private void Start()
    {
        ...
        loginBtn.interactable = true;  //interactable 활성화/비활성화
    }
    ...
    IEnumerator SignUp(UserData userdata)
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Put("http://39.120.26.93:3000/users/add", postData))
        {
            www.method = "POST";
    ...
    IEnumerator SignIn(UserData userdata)
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Put("http://39.120.26.93:3000/users/find", postData))
        {
            www.method = "POST";
            www.SetRequestHeader("Content-Type", "application/json");
            ...
            else
            {
                ...
                else if(result.result == 0)
                {
                    ...
                    errorText.text = "아이디가 존재하지 않습니다. 확인해주세요";
                }
                else if (result.result == 1)
                {
                    Debug.LogError("패스워드가 다릅니다. 확인해주세요");
                    errorText.text = "패스워드가 다릅니다. 확인해주세요";
